...
def main(args):
    ...
    df_src = pd.read_csv(f"./train_split_4.csv")
    video_files = df_src["vid_path"].to_numpy()
    captions, conf_scores, vidnum = [], [], []
    for vidfile in video_files: 
        args.test_video_fname = vidfile
        vidn = vidfile.split("/")[-1].split(".")[0]
        vidnum.append(vidn)
        cap, conf = inference(args, args.test_video_fname, vl_transformer, tokenizer, tensorizer)
        captions.append(cap)
        conf_scores.append(conf)
    df = pd.DataFrame({"vid_num": vidnum, "SwinBERT Captions": captions, "SwinBERT Confidence Scores": conf_scores})
    df.to_csv(f"SwinBERT_split4_train_captions.csv")
