...
def test_IGV(model, test_loader, device, save_to_file):
    model.eval()
    prediction_list = []
    answer_list = []
    ques_keys = []
    ...
    for ques_key, pred, ans in zip(ques_keys, predict_answers, ref_answers):
        results.append([ques_key, pred.item(), ans.item()])
    
    acc_num = acc_num*100.0 / len(ref_answers)
...
def main(model_path="/home/adithyas/11777-videoQA/IGV/weights/IGV_best_model.ckpt", sample_list_path='/home/adithyas/NExT-QA/dataset/nextqa', video_feature_path = '/home/adithyas/NExT-QA/data/feats', save_csv="EIGV_test_results.csv"):
    args = get_args()
    ...
    mem_bank = torch.FloatTensor(test_dataset.all_feats)
    model = HGA(args.ans_num, args.hd,  args.wd, args.drop, args.tau, args.ln ,memory=mem_bank)
    model.load_state_dict(torch.load(model_path))
    model.to(device)
    
    test_acc = test_IGV(model, test_loader, device, save_csv)
    
    
if __name__ == "__main__":
    model_path="/home/adithyas/11777-videoQA/IGV/weights/IGV_best_model.ckpt"
    sample_list_path='/home/adithyas/NExT-QA/dataset/nextqa'
    video_feature_path = '/home/adithyas/NExT-QA/data/feats'
    save_csv="/home/adithyas/11777-videoQA/IGV/IGV_test_results.csv"
    main(model_path, sample_list_path, video_feature_path, save_csv)
