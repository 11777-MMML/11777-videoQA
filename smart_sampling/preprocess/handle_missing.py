...
class VideoLoader(Dataset):
    ...
    def __init__(
        ...
        Args:
        """
        csv_base = '../data/next-dataset'
        self.csv_train = pd.read_csv(os.path.join(csv_base, 'train.csv'))
        self.csv_val = pd.read_csv(os.path.join(csv_base, 'val.csv'))
                
        base_path = os.path.join(path, f"MissingVideos")
...
if __name__ == '__main__':
    ...
    train_video_id = dataset.csv_train['video_id'].values
    val_video_id = dataset.csv_val['video_id'].values
    ...
    with th.no_grad():
        try:
            for batch in tqdm(dataloader):
                ...
                file_name = os.path.basename(video_file[0])
                file_name = os.path.splitext(file_name)[0]

                video_id = int(file_name)

                if video_id not in train_video_id and video_id not in val_video_id:
                    continue
                
                if video_id in train_video_id:
                    output_file = output_file[0]
                    output_file.replace('MissingVideos', 'training_vit_features')
                    output_file = [output_file]
                elif video_id in val_video_id:
                    output_file = output_file[0]
                    output_file.replace('MissingVideos', 'validation_vit_features')
                    output_file = [output_file]
