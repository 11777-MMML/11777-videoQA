...
class EncoderVidHGA(nn.Module):
    ...
    def forward(self, vid_feats, fg_mask=None, extract_vid_feats=False):
        """
...
class VideoQANetwork(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.vid_encoder = EncoderVidHGA(vid_dim, hidden_dim, input_dropout_p=self.dropout, bidirectional=True,
                                         is_gru=False)
        self.qns_encoder = EncoderQns(self.word_dim, hidden_dim, n_layers=1, rnn_dropout_p=0,
                                      input_dropout_p=self.dropout, bidirectional=True, is_gru=False)
    ...
    def forward(self, v, q, q_len, *args):  # videos, qas, qas_lengths, vid_idx, lam_1, lam_2, index, ans
        ...
        if not self.training:
            out = self.fusion_predict(q_global_, torch.repeat_interleave(v_global_, 5, dim=0), q_local_,
                                      torch.repeat_interleave(v_local_, 5, dim=0), q_len, mode='xe')  # bsx5,1
            return out.view(B, 5)

        # in training
        # pretty = pprint.PrettyPrinter(indent=4)
        # pdb.set_trace()
        vid_idx, lam_1, lam_2, index, ans, nb_idxs = args

        # get gt qas
        q_len_gt = q_len.view(B, 5)[torch.arange(B).cuda(), ans]  # B,
        q_feat_gt_global = q_global_.view(B, 5, -1)[torch.arange(B).cuda(), ans, :]  # bs, 512
        q_feat_gt_local = q_local_.view(B, 5, q_local_.size(-2), q_local_.size(-1))[torch.arange(B).cuda(), ans, :,
                          :]  # bs, 37, 512
        ...
        v2 = (1 - lam_1) * v[index] * (fg_mask_[index].unsqueeze(-1)) + (1 - lam_2) * v[index] * (
            bg_mask_[index].unsqueeze(-1))
        ...
        q_local = torch.cat([q_local_.view(B, 5, q_local_.size(-2), q_local_.size(-1)),
                             q_local_.view(B, 5, q_local_.size(-2), q_local_.size(-1))[index]], 1)  # bs, 5x2, 512

        out = self.fusion_predict(q_global.view(-1, q_global.size(-1)), torch.repeat_interleave(v_global, 5 * 2, dim=0), \
                                  q_local.view(-1, q_local.size(-2), q_local.size(-1)),
                                  torch.repeat_interleave(v_local, 5 * 2, dim=0), q_len.view(-1),
                                  mode='xe')  # bsx5x2,1

        ### compute on mixed data ###
        # anchor
        out_anc = self.fusion_predict(q_feat_gt_global, v_global, q_feat_gt_local, v_local, q_len_gt,
                                      mode='cl')  # bs,1853
        fg_mask, bg_mask = self.frame_att(q_feat_gt_global, v_local)  # bs,16

        temp_mem_bank_idx = np.random.choice(range(B), size=5, replace=False).tolist()

        # pos1: sample bg frame, add to original video
        v_pos, (temp_vid_idx, swap_vid_idx, swap_vid_idx_frame) = self.mem_swap(bg_mask, v, vid_idx,
                                                                                temp_mem_bank_idx=temp_mem_bank_idx, nb_idxs=nb_idxs)  # bs, 16, d
        v_local_pos1, v_global_pos1 = self.vid_encoder(v_pos)
        out_pos = self.fusion_predict(q_feat_gt_global, v_global_pos1, q_feat_gt_local, v_local_pos1,
                                      q_len_gt)  # bs,1853

        print_element = dict()
        print_element['vid_idx'] = temp_vid_idx.detach().cpu().numpy().tolist()
        print_element['q_idx'] = ans
        print_element.update({"positive_swap_vid_idx": swap_vid_idx.detach().cpu().numpy().tolist(),
                              "positive_swap_vid_frame": swap_vid_idx_frame.detach().cpu().numpy().tolist()})

        # neg1: rand_V(swap fg) + Q
        v_neg, (temp_vid_idx, swap_vid_idx, swap_vid_idx_frame) = self.mem_swap(fg_mask.repeat(self.num_neg, 1),
                                                                                v.repeat(self.num_neg, 1, 1),
                                                                                vid_idx.repeat(self.num_neg, 1),
                                                                                temp_mem_bank_idx=temp_mem_bank_idx, nb_idxs=nb_idxs.repeat(self.num_neg, 1))  # bsxnum_neg, 16, 4096
        v_local_neg, v_global_neg = self.vid_encoder(v_neg)  # bsxnum_neg, 16, d
        out_neg1 = self.fusion_predict(q_feat_gt_global.repeat(self.num_neg, 1), v_global_neg,
                                       q_feat_gt_local.repeat(self.num_neg, 1, 1), v_local_neg,
                                       q_len_gt.repeat(self.num_neg))  # bsxnum_neg, 512
        print_element.update({"negative_swap_vid_idx": swap_vid_idx.detach().cpu().numpy().tolist(),
                              "negative_swap_vid_frame": swap_vid_idx_frame.detach().cpu().numpy().tolist()})
        ...
        out_neg3 = self.fusion_predict(q_global_neg, v_global.repeat(self.num_neg, 1), q_local_neg,
                                       v_local.repeat(self.num_neg, 1, 1), q_len_neg)

        out_neg = torch.cat((out_neg1, out_neg3), dim=0).view(B, -1, out_neg1.shape[-1])  # bsx(n_neg+n_neg), 512
        return out.view(B, 5 * 2), out_anc, out_pos, out_neg, print_element, temp_mem_bank_idx
