...
class MemBank(nn.Module):
    ...
    def forward(self, bg_mask, vid_feats, vid_idx, temp_mem_bank_idx=[], nb_idxs=None):
        '''
        input:
            bg_mask: bs,16 float
        ...
        #newpart
        weight = vid_feats.new_zeros(bs, self.mem_bank.size(0))
        ...
        sample_idx_mask = sample_idx * (bg_mask)
        temp_indices = temp_mem_bank_idx
        temp_indices.append(0)
        temp_vid_idx = vid_idx[temp_indices]
        # print(f'sample idx mask: {sample_idx_mask[temp_indices].shape}')
        swap_video_idx = torch.floor((sample_idx_mask[temp_indices])/ 16)
        swap_video_idx_frame = (sample_idx_mask[temp_indices]) % 16
        # print(f'{[temp_vid_idx, swap_video_idx, swap_video_idx_frame]}--vid_idx: {temp_vid_idx}, swapped video idx: {swap_video_idx}, frame_idx from swapped video: {swap_video_idx_frame}')
        # mix fg/bg, indicated by bg_mask
        vid_feats_new = vid_feats*((1-bg_mask).unsqueeze(-1)) + sampled_bg*(bg_mask.unsqueeze(-1)) 

        return vid_feats_new, (temp_vid_idx, swap_video_idx, swap_video_idx_frame)

if __name__ == "__main__":
    print('main start')
    vid_feats=torch.ones(2,16,4096).float()
    answers=None
    qns_keys=None
    vid_idx=torch.tensor([1,2],dtype=torch.int64)
    bg_mask=(torch.rand(2,5)>0.5).float()
    model=MemBank()
    out = model(bg_mask, vid_feats, vid_idx, nb_idxs=torch.tensor([[10,24,50],[80,240,380]]))
    print(out.type())
