...
class MemBank(nn.Module):
    ...
    def forward(self, bg_mask, vid_feats, vid_idx):
        ...
        sample_idx_mask = sample_idx * (bg_mask.unsqueeze(-1))
        temp_indices = np.random.choice(range(bs), size=10, replace=False)
        temp_indices.append(0)
        temp_vid_idx = vid_idx[temp_indices]
        swap_video_idx = floor(sample_idx_mask[temp_indices] / 16)
