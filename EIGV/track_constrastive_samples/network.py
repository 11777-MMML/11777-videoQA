...
class VideoQANetwork(nn.Module):
    ...
    def forward(self, v, q, q_len, *args): #videos, qas, qas_lengths, vid_idx, lam_1, lam_2, index, ans
        ...
        v_pos, (temp_vid_idx, swap_vid_idx, swap_vid_idx_frame) = self.mem_swap(bg_mask, v, vid_idx, temp_mem_bank_idx=temp_mem_bank_idx) # bs, 16, d
        ...
        print_element.update({"positive_swap_vid_idx": swap_vid_idx.detach().cpu().numpy().tolist(), "positive_swap_vid_frame": swap_vid_idx_frame.detach().cpu().numpy().tolist()})
       
        
        # neg1: rand_V(swap fg) + Q
        v_neg, (temp_vid_idx, swap_vid_idx, swap_vid_idx_frame) = self.mem_swap(fg_mask.repeat(self.num_neg,1), v.repeat(self.num_neg,1,1), vid_idx.repeat(self.num_neg,1), temp_mem_bank_idx=temp_mem_bank_idx) # bsxnum_neg, 16, 4096
        ...
        print_element.update({"negative_swap_vid_idx": swap_vid_idx.detach().cpu().numpy().tolist(), "negative_swap_vid_frame": swap_vid_idx_frame.detach().cpu().numpy().tolist()})
        ...
        return out.view(B,5*2), out_anc, out_pos, out_neg, print_element
