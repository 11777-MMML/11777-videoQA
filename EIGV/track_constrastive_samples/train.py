...
def train(model, optimizer, train_loader, xe, nce, device, epoch=0):
    ...
    for iter, inputs in enumerate(train_loader):
        ...
        out, out_anc, out_pos, out_neg, iter_print_element = model(videos, qas, qas_lengths, vid_idx, lam_1, lam_2, index, ans)
        model.zero_grad()
        # xe loss
    ...
    return epoch_loss / total_step, epoch_xe_loss/ total_step, epoch_nce_loss/ total_step, acc_num*100.0 / len(ref_answers), epoch_print_dict
...
def update_prints(prints: list, train_data):
    ...
    for printt in prints:
        for vid_idx, positive_swap_vid_idx, positive_swap_vid_frame, negative_swap_vid_idx, negative_swap_vid_frame in zip(printt['vid_idx'], printt['positive_swap_vid_idx'], printt['positive_swap_vid_frame'], printt['negative_swap_vid_idx'],printt['negative_swap_vid_frame']):
            new_printt = dict()
            ...
            for idx in range(0, len(positive_swap_vid_idx)):
                if not (int(positive_swap_vid_idx[idx]) == 0):
                    new_printt['positive_swap_vid_idx'].append(idx2vid[int(positive_swap_vid_idx[idx])]+':'+str(int(positive_swap_vid_frame[idx])))
                else:
            ...
            for idx in range(0, len(negative_swap_vid_idx)):
                if not (int(negative_swap_vid_idx[idx]) == 0):
                    new_printt['negative_swap_vid_idx'].append(idx2vid[int(negative_swap_vid_idx[idx])]+':'+str(int(negative_swap_vid_frame[idx])))
                else:
