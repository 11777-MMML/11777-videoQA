...
class MemBank(nn.Module):
    ...
    def forward(self, bg_mask, vid_feats, vid_idx, is_v_pos=False):
        '''
        input:
            bg_mask: bs,16 float
        ...
        if is_v_pos:
            # suppressing creation of positive sample with passing a 0s background all the time, so maintaining the causal part responsible for the answer, but not really varying the background
            sampled_bg = torch.zeros(sampled_bg.shape).to(vid_feats.device)

        # mix fg/bg, indicated by bg_mask
        vid_feats_new = vid_feats*((1-bg_mask).unsqueeze(-1)) + sampled_bg*(bg_mask.unsqueeze(-1)).to(vid_feats.device)
