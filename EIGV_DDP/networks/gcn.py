...
class GraphConvolution(Module):
    ...
    def __init__(self, in_features, out_features, device):
        ...
        self.layer_norm = nn.LayerNorm(out_features, elementwise_affine=False).to(device)

    def forward(self, input, adj):
        # self.weight of shape (hidden_size, hidden_size)
        support = self.weight(input).to(self.device)
        output = torch.bmm(adj.to(self.device), support).to(self.device)
        output = self.layer_norm(output).to(self.device)
        return output
...
class GCN(nn.Module):
    ...
    def __init__(
            self, input_size, hidden_size, num_classes, num_layers=1, device="cuda", dropout=0.1):
        super(GCN, self).__init__()
        self.device = device
        self.layers = nn.ModuleList()
        model = GraphConvolution(hidden_size, num_classes, device)
    ...
    def forward(self, x, adj):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = self.dropout(self.relu(layer(x, adj))).to(self.device)
