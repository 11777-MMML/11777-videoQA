...
class VideoQANetwork(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.device = self.tau_gumbel = kwargs.pop("device")
        hidden_dim = self.module_dim
        
        vid_dim = self.app_pool5_dim*2
        self.vid_encoder = EncoderVidHGA(vid_dim, hidden_dim, input_dropout_p=self.dropout,bidirectional=True, is_gru=False).to(self.device)
        self.qns_encoder = EncoderQns(self.word_dim, hidden_dim, n_layers=1,rnn_dropout_p=0, input_dropout_p=self.dropout, bidirectional=True, is_gru=False).to(self.device)

        hidden_size = self.vid_encoder.dim_hidden*2 

        self.fg_att = AttentionScore(hidden_size, self.device).to(self.device)
        self.bg_att = AttentionScore(hidden_size, self.device).to(self.device)

        self.mem_swap = MemBank(self.device).to(self.device)
        
        self.adj_learner = AdjLearner(hidden_size, hidden_size, dropout=self.dropout).to(self.device)
        ...
        self.atten_pool = nn.Sequential(
            ...
            nn.Softmax(dim=-2)).to(self.device)

        # self.agg_pools = nn.Sequential(*[nn.MaxPool1d(kernel_size=i+2, stride=1) for i in range(n_pos-1)])

        self.global_fusion = fusions.Block([hidden_size, hidden_size], hidden_size, dropout_input=self.dropout).to(self.device)
        self.fusion = fusions.Block([hidden_size, hidden_size], hidden_size).to(self.device)
        self.project_head = nn.Sequential(nn.ReLU(), nn.Linear(hidden_size,hidden_size)).to(self.device)
        self.decoder=nn.Linear(hidden_size, 1).to(self.device) # ans_num+<unk>
    ...
    def forward(self, v, q, q_len, *args): #videos, qas, qas_lengths, vid_idx, lam_1, lam_2, index, ans
        ...
        # get gt qas
        q_len_gt = q_len.view(B,5)[torch.arange(B).to(self.device),ans] # B,
        q_feat_gt_global = q_global_.view(B, 5, -1)[torch.arange(B).to(self.device),ans,:] # bs, 512
        q_feat_gt_local = q_local_.view(B, 5, q_local_.size(-2), q_local_.size(-1))[torch.arange(B).to(self.device),ans,:, :] # bs, 37, 512
    ...
    def fusion_predict(self, q_global, v_global, q_local, v_local, q_len, mode='cl'):
        # ## gcn
        adj = self.adj_learner(q_local, v_local).to(self.device)
        q_v_inputs = torch.cat((q_local, v_local), dim=1).to(self.device)
        q_v_local=self.gcn(q_v_inputs, adj).to(self.device)
        ...
        ## fusion
        global_out = self.global_fusion((q_global, v_global)).to(self.device)
        out = self.fusion((global_out, local_out)).squeeze().to(self.device) # bs x hidden
        
        out_cl=self.project_head(out).to(self.device)
        out_xe=self.decoder(out).to(self.device)
