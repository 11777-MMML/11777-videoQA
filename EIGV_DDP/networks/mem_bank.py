...
class AttentionScore(nn.Module):
    ...
    def __init__(self, hidden_size, device, dropout_p=0.0):
        super().__init__()
        self.device = device
        ...
        self.linear_q = nn.Linear(hidden_size, hidden_size).to(self.device)
        self.linear_k = nn.Linear(hidden_size, hidden_size).to(self.device)
...
class MemBank(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device 
        bank_path='/mnt/adithya/action_caption_dataset/action_caption_dataset/video_feats/video_feats_32/train_pre.hdf5'
        with h5py.File(bank_path) as f:
            self.mem_bank=torch.Tensor(f['feat'][:]).to(self.device)
